#pragma once
// LoRaTrace RX — per-profile RF parameter tables. See docs/DESIGN.md §3 for the
...
// phase (docs/DESIGN.md §9) needs them — see CLAUDE.md house rule against
...
// docs/DESIGN.md §3/§7's job, not this file's — don't re-derive it here.
...
// duty-cycled/CAD scanning in docs/DESIGN.md §4 (phase 4+) — revisit there.
...
// (official docs + multiple corroborating sources, docs/DESIGN.md §3).
//
...
// caught by DISCOVERY_SWEEP (docs/DESIGN.md §4, build-order Phase 8).
...
// sites agree, docs/DESIGN.md §3). MeshCore has no slot-hashing scheme — one
// frequency covers the whole regional community (docs/DESIGN.md §3).
...
// Maps a mission profile to its HOME_LISTEN channel table (docs/DESIGN.md §5:
...
// Phase 9) and never get one, so they fall back to the Meshtastic default
...
// default, see docs/history/CHANGELOG.md). RETICULUM/GENERAL_EXPLORATION
...
// --- Not yet parameterized — build-order Phases 8/9, docs/DESIGN.md §3/§9 ---
//
// MeshCore legacy (pre-migration, may still be in the wild): ~915MHz,
// SF11, BW250. Coding rate is genuinely unspecified in docs/DESIGN.md ("—" in
...
// Handled entirely by ENERGY_SWEEP (build-order Phase 9) — never gets a
// ChannelParams entry.
//
...
// docs/DESIGN.md §5 — kept as history, not current behaviour.

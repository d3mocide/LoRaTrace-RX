...
|---|---|---|
| **12 — Survey truth** | **Engineering** | Define coverage vocabulary and persistent per-survey evidence, then add bounded Focus Survey. The [design-entry and acceptance plan](research/phase12-survey-truth-design.md) locks the one-bin first slice and controlled matrix. Two gates closed as decisions 2026-09-06: the radio-away budget is **refused** while Focus stays one-Enter-one-pass, and Focus **reports coverage and never activity** — the rule that survived measurement depends on link quality the device cannot know. `sampled`/`repeated` thresholds remain the last measurement, now descriptive rather than carrying an inference. |

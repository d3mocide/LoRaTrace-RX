...
void test_focus_working_state_stays_below_one_result_budget() {
    ...
    // 44 -> 48 on 2026-09-07: skipped_samples adds 2 bytes and alignment
    // rounds the struct to 48 (audit A22, so a pass that abandoned late slots
    // says so rather than reporting a spacing it never had).
    //
    // That is exactly the 48B/record evaluation ceiling energy_observation.h
    // documents, with no headroom left. The next field added here needs the
    // ceiling revisited deliberately, not raised to make this assert pass.
    TEST_ASSERT_EQUAL_size_t(48, sizeof(FocusObservation));
    TEST_ASSERT_TRUE(sizeof(FocusObservation) <= 48);
    TEST_ASSERT_EQUAL_size_t(196, sizeof(FocusRssiHistogram) + sizeof(FocusObservation));
}
...
void test_focus_csv_with_fix_persists_coverage_and_raw_counts() {
    ...
    TEST_ASSERT_EQUAL_STRING(
        ...
        "912.750,1,1,500,470,4,4,-92.0,-85.0,-85.0,0,sampled,complete,1,0,0,0,0", row);
}
